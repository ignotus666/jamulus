...
// number of stable statistics windows needed to reach full aggressive decay
#define ADAPTIVE_STABILITY_THRESHOLD 4
...
protected:
    ...
    // adaptive decay: interpolate between conservative and aggressive IIR
    // down-weights based on observed network stability
    double dConservativeDownNormal; // conservative (upstream) IIR down-weight
    double dAggressiveDownNormal;   // aggressive floor IIR down-weight
    double dAdaptiveDecayFactor;    // 0.0 = conservative, 1.0 = full aggressive
    int    iStabilityCounter;       // consecutive stable statistics windows
    int    iPrevAutoBufferResult;   // previous auto-setting for stability detection
};

...
namespace
{
static bool IsLv2Uri ( const std::string& sPath ) { return sPath.find ( "http://" ) == 0 || sPath.find ( "https://" ) == 0; }
} // namespace

CPluginHost::CPluginHost() { StartLoaderThread(); }
...
#ifdef HAVE_CARLA
    if ( carlaAdapterHandle && carla_adapter_get_plugin_count_lock_free ( carlaAdapterHandle ) > 0 )
        return true;
#endif
...
{
    QWriteLocker lg ( &rwLockPlugins );
    auto         it = std::find_if ( vecPlugins.begin(), vecPlugins.end(), [iPluginId] ( const PluginEntry& e ) { return e.id == iPluginId; } );
    ...
    if ( it->closeEditor && it->instance )
    {
        ...
        loaderCommands.push ( [inst, this] {
            try
            {
                ...
                auto jt = std::find_if ( vecPlugins.begin(), vecPlugins.end(), [inst] ( const PluginEntry& e ) { return e.instance == inst; } );
...
void CPluginHost::IdlePluginEditors()
{
    ...
    if ( carlaAdapterHandle )
    {
        int ticks = audioActiveTicks.load ( std::memory_order_relaxed );
        if ( ticks > 0 )
        {
            audioActiveTicks.store ( ticks - 1, std::memory_order_relaxed );
            carla_adapter_idle ( carlaAdapterHandle );
        }
        else if ( carla_adapter_is_restoring ( carlaAdapterHandle ) )
        {
            carla_adapter_idle ( carlaAdapterHandle );
        }
    }
...
int CPluginHost::LoadPluginImpl ( const std::string& sPath )
{
    ...
    plugin_handle_t inst          = lv2_create_from_uri ( sPath.c_str(), GetSampleRateHz(), GetBlockSizeFrames(), iHostChannels );
    if ( !inst )
    {
...
{
    audioActiveTicks.store ( 30, std::memory_order_relaxed );
    ...
    if ( carlaAdapterHandle )
    {
        carla_adapter_process ( carlaAdapterHandle,
                                buffer.data(),
                                iFrames,
                                iChannels,
                                localMidiEvents.data(),
                                static_cast<int> ( localMidiEvents.size() ) );
    }
    else
#endif
    {
